...
| [org.json (JSON-java)](https://github.com/stleary/JSON-java) (`org.json:json`) | 20260814 | Public Domain | Test-only, `:feature:routing`. `GeoCategories` parses `org.json.JSONObject` — already on every Android device, so the *production* path adds no new dependency at all (brief's own step 3). But `GeoCategoriesTest` runs as a plain JVM unit test against AGP's `android.jar` stub, whose `org.json` methods throw `RuntimeException: Method optJSONArray in org.json.JSONObject not mocked` at runtime instead of parsing anything — confirmed empirically: the brief's own "reads codes and rule counts" test failed with exactly that exception before this line was added, while the two failure-path tests passed vacuously (the same exception was silently swallowed by `GeoCategories.read`'s own catch-all). This is JSON-java's real reference implementation, resolved to the unit-test classpath ahead of the android.jar stub of the same class names, and it never reaches the shipped APK. Licence verified from the artifact's own POM (`<license><name>Public Domain</name></license>`) and the upstream `LICENSE` file, not a project README — the historical "JSON License" ("shall be used for Good, not Evil") was retired in favour of an unconditional public-domain dedication. |
