...
| [libXray](https://github.com/XTLS/libXray) | v26.7.11, **patched** | MIT | Attribution only. **Modified**: `third_party/libxray-patches/0001-restore-invoke-env.patch` re-applies upstream's own PR #133 (26 lines, `invoke.go` + `invoke_model.go`) to restore the `env` object on the invoke request. Without it `geoip:`/`geosite:`/`ext:` rules cannot resolve on Android at all — Go is started with an empty environment there, so `Os.setenv` from Java never reaches it (research §2b). MIT permits modification with the notice retained, which the patch does not touch; the AAR is built by `scripts/build-libxray.sh`, not downloaded. Upstream removed this in PR #134 three days before v26.7.11 was tagged; when it returns, delete the patch and the build reverts to the official release. |
