...
| [hev-socks5-tunnel](https://github.com/heiher/hev-socks5-tunnel) | 2.16.0, submodule commit `0a05221275a51a884d93328c55fc2fbc9e9b6974`, parent-owned patch | MIT | Attribution only. Compiled from source into `libtun2socks.so`. `third_party/hev-patches/0001-nonblocking-pending-stop.patch` replaces `quit`'s readiness busy-wait with one atomic pending-stop lifecycle so a stop arriving before event initialization cannot wedge JNI teardown; the JNI mutex and exactly-once join ownership remain local to Subspace. `:service:preparePatchedHev` fails unless the submodule is clean and exactly at the pinned upstream commit, applies the patch into a generated build tree, verifies the four patched source hashes, and every native configure task compiles that tree. Upstream `src/hev-jni.c` is excluded because it registers JNI natives against upstream's own app class and aborts any other process that loads it; see `service/src/main/jni/Android.mk`. Nested vendored dependencies (yaml, lwip, hev-task-system) remain unmodified. |
