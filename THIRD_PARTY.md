...
| [kotest-assertions-core](https://github.com/kotest/kotest) | 5.9.1 | Apache-2.0 | Test-only. Assertions in every module's unit tests, and in `:core:xray`'s and `:core:ui`'s `androidTest` — wired there explicitly because the convention plugin only adds it to `testImplementation`. Assertions only; the kotest *runner* is deliberately not used, so tests stay plain JUnit 4. |
