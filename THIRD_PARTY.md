...
| [hev-socks5-tunnel](https://github.com/heiher/hev-socks5-tunnel) | 2.16.0, submodule commit `0a05221275a51a884d93328c55fc2fbc9e9b6974`, parent-owned patch | MIT | Attribution only. Compiled from source into `libtun2socks.so`. `third_party/hev-patches/0001-nonblocking-pending-stop.patch` replaces `quit`'s readiness busy-wait with one atomic pending-stop lifecycle so a stop arriving before event initialization cannot wedge JNI teardown; the JNI mutex and exactly-once join ownership remain local to Subspace. `:service:preparePatchedHev` fails unless the outer submodule and all four nested submodules match their pinned commits with no tracked changes, exports only their tracked indexes, verifies the complete inventory before and after applying the patch, and permits exactly the four reviewed source hashes to differ. Parent-tracked `scripts/HevPrepareLock.java` uses Java 17's OS-backed exclusive `FileChannel.lock()` across validation, generation, publication, and active-version selection; process death releases it without PID or stale-owner logic. Immutable real version directories are selected only through a strictly validated relative `current` symlink and published atomically, so concurrent builds reuse a verified tree without a missing-output window. Every native configure task compiles only `current`; `:service:testHevSourceIsolation` uses disposable local shared clones to test outer/nested tracked, untracked, and ignored mutations, concurrent contenders, holder termination/crash recovery, and malformed lock/output symlinks without touching live Git metadata or external sentinels. Upstream `src/hev-jni.c` is excluded because it registers JNI natives against upstream's own app class and aborts any other process that loads it; see `service/src/main/jni/Android.mk`. Nested vendored dependencies (yaml, lwip, hev-task-system, and hev-socks5-core) remain unmodified. |
