...
lock_wrapper="$script_dir/with-hev-prepare-lock.pl"

lock_fd="${SUBSPACE_HEV_PREPARE_LOCK_FD:-}"
if [[ ! "$lock_fd" =~ ^[0-9]+$ ]]; then
    echo "HEV preparation requires an inherited kernel lock descriptor" >&2
    exit 70
fi
...
pause_fifo="${SUBSPACE_HEV_PREPARE_TEST_PAUSE_FIFO:-}"
pause_ready="${SUBSPACE_HEV_PREPARE_TEST_PAUSE_READY:-}"
unset SUBSPACE_HEV_PREPARE_TEST_PAUSE_FIFO SUBSPACE_HEV_PREPARE_TEST_PAUSE_READY
if [[ -n "$pause_fifo" || -n "$pause_ready" ]]; then
    if [[ -z "$pause_fifo" || -z "$pause_ready" || ! -p "$pause_fifo" ||
        -L "$pause_ready" || -e "$pause_ready" ]]; then
        echo "invalid deterministic HEV preparation pause seam" >&2
        exit 71
    fi
    : >"$pause_ready"
    IFS= read -r _ <"$pause_fifo" || true
fi
